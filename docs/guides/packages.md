...
- **Objetivos:** Inventariar paquetes por perfil (`base`, `developer`, `production`, …) para revisar el monolito recortado sin abrir cada JSON.
- **Estructura:** Leyenda → tablas por perfil → notas AUR.
- **Contenido a integrar según contexto:** Sincroniza esta guía cuando cambie `configs/*.json`. No copies un catálogo de otro distro o producto.
